...
- **`httpbad` / `http_malformed` now includes requests answered with HTTP 400.** `0.14.1` counted only connections dropped for incomplete or unroutable request heads. The tick field (`httpbad`) and `status.json` field (`http_malformed`) retain their names, but the same traffic can produce higher counts after upgrading. Adjust alerts to include rejected request lines, lengths, paths, and ranges, rather than interpreting every increment as a dropped connection.
- **The tick line and `status.json` gain `http_send_err`.** It counts each `/have` or `/data` response interrupted during header or body sending once, including deadline expiry and local failures after a success header. `0.14.1` had no counter for these failures. Update strict JSON and positional tick parsers; treat a missing field on older nodes as unavailable, not zero failures. `httpok` and `serve_mib` still count accepted success headers and advertised bytes, not complete delivery. Send failures can overlap `httpok` and do not count as `http5xx`; use the new counter alongside those existing signals.
...
- **Restore-drill stub tests ignore an operator's exported `MF_*` settings.** `scripts/test_dr_restore_drill.sh` previously inherited `MF_DRILL_SCRATCH`, `MF_DRILL_CLONE_MP`, `MF_DRILL_MAX_SNAP_AGE`, and the other NAS knobs, so running `check.sh` from a drill operator's shell could redirect fixture writes off the fixture tree, fail a stale-mount preflight, or flip expected alarms; the stub `zfs` also `rm -rf`-ed whatever mountpoint the environment named. The suite now pins every per-test `MF_` value and refuses clone paths outside its fixture tree.
- **`scripts/test_hot_reload.sh` kills the held reader on early failure and preflights stale state.** An assertion failure before the handover left the background reader polling forever with its fd open; the EXIT trap now reaps it, the readiness wait aborts when the reader dies, and a leftover mount or port listener from a previous run fails by name instead of as a wrong-byte error mid-test.
...
- **Restore-drill clones are read-only and unshared.** `scripts/dr_restore_drill.sh` cloned with only a mountpoint override, so the `MF_DRILL_KEEP` inspection copy and any clone left by a failed cleanup inherited the pool's `readonly` and NFS/SMB share properties. The clone command now sets `readonly=on`, `sharenfs=off`, and `sharesmb=off`, and the stub suite rejects a drill that drops them.
...
conformance, and robustness fixes found by reading, with tests for the ones
a unit test can reach. Second, releases: static single-file binaries for
both shipped targets (libfuse3 vendored and compiled in), a tag-triggered
release workflow where none existed, a wider benchmark sweep with the
engine-shaped I/O the old one could not see, and workflow diagrams beside
the prose they summarize. No wire, on-disk, or CLI change, so a mixed fleet
with `0.8.0` peers still fills, fills verifiably, and hands over between
these builds.

### Native Zig abstractions over raw libc calls - 2026-09-03
- **Everything Zig's own layers can express no longer goes through libc.** A sweep of the tree found 116 direct `std.c`/`std.os`/`std.posix` uses plus the C-door call sites; the ones with native coverage moved: `/proc/self/exe` resolves through `Io.Dir.readLinkAbsolute` (`selfExe`), lease-sweep mtime aging in the tests goes through the new `sys.touchPath` (`Io.File.setTimestamps` — the same utimensat, behind the Io abstraction, symlink semantics unchanged), path canonicalization uses `Io.Dir.realPathFileAlloc` (`sys.realpathAlloc` now takes `io`; `resolveOriginDir`/`ensureDirReal` thread it), the daemon's pid is centralized in `sys.pidSelf` instead of nineteen scattered raw `getpid` calls, and clocks and sleeps already rode `std.Io.Clock`/`std.Io.sleep`. The remaining low-level calls are enumerated and justified in the `sys.zig` header: libfuse3's session and reply calls are C by design, and `sendfile`, punch-hole, `statfs`, `O_NOFOLLOW` opens, sealed memfds, socket deadlines, `fcntl`, `environ`, `errno`, and 0.16's missing `std.crypto.random` have no `std` coverage.

### Static single-file release builds - 2026-09-03
- **`zig build -Dfuse-static` compiles the vendored libfuse3 into the binary.** The release artifacts promised "one Zig binary per node", but every shipped binary dynamically linked the distro `libfuse3.so` — and no usable static `libfuse3.a` exists for musl anywhere (the glibc-built `.a` in the vendored arm64 `-dev` package is full of glibc-only symbols). The source of libfuse 3.16.2 is now vendored under `.deps/libfuse3-3.16.2/` (upstream tarball, digest-pinned like the arm64 `.deb`s, one hand-written `fuse_config.h` replacing the header meson generates), and `-Dfuse-static` compiles those sixteen files straight into the executable. The release targets are `x86_64-linux-musl` and `aarch64-linux-musl`: static-PIE (ASLR kept), RELRO, non-executable stack, ~2.5 MiB, verified on this machine with live mounts.
- **Musl needed four portability fixes, all verified against both libcs.** musl's headers defeat translate-c twice — `struct timespec` and `struct statvfs` carry anonymous zero-width bitfields, which translate-c demotes to opaque types, cascading into `struct stat` — so musl targets translate through `src/c_musl.h` (pre-defines `timespec`, pinned to the LP64 little-endian layout) with an ABI-identical `sys/statvfs.h` shim (`src/c-musl-shim/`); musl's `sigaction` union spells its handler `__sa_handler` where glibc says `__sigaction_handler`; and zig's musl libc.a ships no `renameat2` wrapper, so `sys.renameAt2` issues the raw syscall for both libcs.
- **The FUSE `statfs` answer is built from `statfs(2)`, not `statvfs(2)`.** musl's `struct statvfs` is untranslatable (the bitfield above), and `statfs` is portable across both libcs; `mf_statfs` now maps the counters explicitly (glibc signs statfs's word fields, musl does not; `f_favail` derives from `f_ffree` the way both libc statvfs wrappers do; `f_fsid` moves as the 8 bytes both spellings occupy).
- **`scrubPskEnv` blanks the inline PSK in place instead of calling `unsetenv`.** start.zig hands std an environment block that *aliases* the libc environ array, and libc's unsetenv compacts that array in place — parking its NULL terminator inside a slot std still counts as live. The first `std.log` after the scrub then dereferenced that NULL on musl static builds (and the trace printer deadlocked re-entering the environ lock mid-scan): every mount with `MODELFS_PSK_VALUE` set segfaulted at startup. Overwriting the entry bytes keeps the array's shape, leaves children a meaningless `XXXX…` variable instead of an empty known knob, and actually wipes the secret, which unsetenv never did.

### Release pipeline - 2026-09-03
- **Tag pushes now publish.** Nothing triggered on a tag before: all nine releases shipped zero artifacts, and the de-facto process was a hand-run build. The new `release` workflow (`.github/workflows/release.yml`) fires on `v*`, builds both static targets through `scripts/build_static.sh` (which asserts the ELF is static-PIE, has no `PT_INTERP`, names no `DT_NEEDED`, and carries `PT_GNU_RELRO` — `assert_static_elf` in `scripts/lib.sh`, raw `od` bytes like its aarch64 sibling), refuses a tag that does not name `build.zig.zon`'s version, and attaches the two binaries plus `SHA256SUMS` to a GitHub release. All actions are SHA-pinned and inventoried in `sbom.cdx.json`.

### Benchmarks: more chunk sizes, and the paths engines actually exercise - 2026-09-03
- **The sweep now reaches 128M and 256M**, and a new benchmark decouples file size from piece size: a fixed 256 MiB file read warm through the FUSE mount, written through it, and fetched as a 64 MiB peer-style HTTP range, across all eleven piece sizes (`run_engine_io_vs_piece_size_benchmark`). The old sweep ties the file size to the piece size — one piece per file — so it could not see what a real model's grid does; the new one shows the trade directly (on the run that regenerated the docs, write-through is flat at ~1.2 GB/s while the HTTP span slows toward the big chunks). Timed values take the best of a few identical passes instead of a single shot. `docs/benchmarks.md` and `docs/figures/` are regenerated by the script (`fig4`, `fig5` added), as designed.

### Documentation: workflow diagrams and naming consistency - 2026-09-03
- **Four mermaid workflow diagrams** now sit beside the prose they summarize: the write-through path and its mark discipline, the cache cull and reap cycle, the `modelfs update` handover (token, sealed memfd, `FUSE_INIT` replay), and the `modelfs pull` skip/download/verify flow.
- **`docs/THREAT_MODEL.md` is `docs/threat-model.md`** — the one file-name outlier against docs/' lowercase-kebab convention; every live reference moved with it (`check.sh`'s tag gate, CONTRIBUTING, AGENTS.md, cross-links), and CHANGELOG history keeps its original paths. Root-level `SECURITY.md`/`README.md`/`CHANGELOG.md` etc. stay uppercase on purpose: tooling recognizes those names.

### Cache growth no longer widens a short tail piece's mark - 2026-09-03
- **A write-through grow past a non-piece-aligned size now drops the old last piece's mark instead of silently widening it.** `cacheFillIdentified` (and `mf_write`'s lag fallback) preserved every mark across a size grow, but when the old size was not piece-aligned the old last piece is short: its bit meant "bytes `[k·ps, old_size)` are cached", and the grow redefined it to claim the full piece while the cache fd never held the gap. A write landing past the gap marks nothing (`fullCover` is empty there), so a warm read answered the hole with zeros as model weights — the exact corruption every other guard in `store.zig` exists to prevent. The new `Store.dropWideningPieceMark` clears that one mark and its stale digest (persisted via `saveBits`) before the size moves; the landing write re-marks the piece through `fullCover` when it actually completes it. Pinned by a regression test that hydrates the short tail, appends past the gap, and asserts `rangeFilled` stays false.

### Reads and transfers of dying entries serve origin, not holes - 2026-09-03
- **`readServed` refuses a dead entry.** A forget (unlink, rename-over) racing a miss read left `hydratePiece` reporting success while the pieces it was supposed to hydrate stayed sparse holes in the surviving cache fd, and the warm-read pread answered them as zeros behind a successful reply. Such a read now goes straight to `originPread` (ENOENT for an unlinked name is the correct answer).
- **The idle-fd reaper no longer closes a cache fd under an in-flight transfer.** `reapIdle`'s phase 2 was the one fd-closing path without the `xfer` gate `punchPiece`, `truncateCacheFd`, and `copyIntoCache` all take; a future xfer user that opened before stamping would have had its descriptor recycled mid-read. The entry stays pinned and the next round reaps it.
- **A `/have` body send now carries the same whole-body deadline as `/data`.** `serveHave` sent the snapshot with one unbounded `writeAll`; because `SO_SNDTIMEO` resets on every drained byte, a receiver trickling one byte per window could pin an inflight slot (thread, socket, snapshot) forever, and sixteen of those deaden the peer service. The send is chunked through `armChunkTimeout`/`bodyDeadlineFor` like `streamRange`.
- **Requests that declare a body are drained, bounded, before the reply.** Closing with unread receive data sends RST instead of FIN, and an RST discards the client's unread response — the 401/405 replies that exist to educate exactly those clients never landed. `drainDeclaredBody` caps the drain at 64 KiB and 10 s so it cannot become a new way to hold a slot.
- **`ll_setattr` answers ENOSYS before mutating.** A chown+chmod (or truncate+utimes) applied the supported subset and then failed, leaving the caller an error whose mode change stuck; the unsupported-bit check moved ahead of `mf_chmod`/`mf_truncate`.
- **`mf_create` honors `O_EXCL` at the origin.** The caller's flags were dropped, so an exclusive creator racing another node's file truncated it out of existence instead of failing EEXIST; the flags now reach the origin open through a `fiFlags` accessor (offset 0 of the opaque `fuse_file_info` head, beside the existing bitfield accessors).

### Handover and `modelfs pull` hardening - 2026-09-03
- **The handover state memfd is closed as soon as it is read.** `cmdHandover` never closed `state_fd`, so the sealed memfd carrying the raw PSK sat at `/proc/<pid>/fd` for the replacement daemon's whole lifetime — the exposure the memfd design exists to bound to the exec moment. The state blob is also `secureZero`d before its free in `readStateFd`, like every other PSK-holding buffer.
- **Handover JSON escapes C0 control bytes.** `jsonStr` passed them through raw, so a control byte in a mountpoint published a document `std.json` refuses: the replacement image's decode failed and, with the req-decode failure swallowed, `modelfs update` of that mount silently timed out forever. `encodeReq` escapes through the same gate instead of raw `allocPrint`.
- **A failed `modelfs update` no longer leaves `update.req` behind.** The kill-failure and timeout exits now unlink it, so a later SIGUSR2 cannot replay an update nobody asked for.
- **The pull skip check lstats instead of stats.** `hf.pull` counted a planted symlink of the right length as "already present", and since every reader opens `O_NOFOLLOW`, that name then failed ELOOP forever — re-running pull could never heal it. Only a regular file at the listed size skips; anything else is replaced through the `.part` rename.
- **The listing size cap holds while the body streams.** The 8 MiB bound was checked after the whole listing landed in a growing buffer, so a hostile endpoint could drive unbounded allocation before any check ran; the fetch now writes into a fixed `max_listing_bytes` buffer whose overflow surfaces as `ListingTooLarge` mid-stream.
- **A short (or over-long) download is refused instead of renamed.** `fetchOne` never compared the landed byte count against the listed size, so a truncated 200 became the real name and the next run's size check counted it as already there. `modelfs pull` now names the mismatch and resumes on rerun.
- **An oversized `HF_TOKEN` is named, not baffling.** The env token was uncapped and the bearer buffer was 512 bytes, so a long value surfaced as `NoSpaceLeft`; the env value is now held to `max_token_bytes` (as the token file always was), the buffer is sized to it, and an OOM in `loadToken` propagates instead of masquerading as an anonymous pull against a private repo.

### Conformance and platform fixes - 2026-09-03
- **`sys.chmod` falls back through `/proc/self/fd` when `fchmodat(AT_EMPTY_PATH)` is unavailable.** That form needs fchmodat2 (kernel 6.6+) or a new-enough libc; older stacks answered EINVAL/ENOSYS and every client chmod through the mount failed. Other errnos still return as-is.
- **`ioRead` retries EINTR** like every read wrapper in `sys.zig`, so a signal landing in libfuse's custom-io read cannot tear down the session on a stack that treats `-EINTR` as a transport error.
- **Manifest publishes use a per-thread temp name.** Two concurrent closes of one entry shared one `.tmp`, and the second `O_TRUNC` open could tear the first's half-written temp before its rename landed; each publish now stages under `.tmp.<tid>`.
- **Lease addresses are held to the dialable gate.** `Catalog.pushPath` accepted `0.0.0.0` and `255.255.255.255` (`parseV4` admits them, `isDialableHost` refuses), letting a planted lease occupy a path slot that `fillFromPeers` burns a dial-timeout on. `formatLease` also enforces the `leaseJsonClean` contract itself, so a future caller that skips its own validation fails its publish instead of sending a lease every peer's parser rejects.
- **Exit codes from libfuse are clamped to u8** (`fuse_fs.run`/`attach` returns), so an out-of-range status cannot panic the `@intCast` in safe builds.

### Harness and docs drift - 2026-09-03
- **The `MF_` knob inventory in `scripts/lib.sh` is complete again**: `MF_HOTRELOAD_PORT`, `MF_SYNCOID_SRC`, `MF_SYNCOID_DEST`, and `MF_DRILL_DATASET` were in use but missing from the list.
- **The DR scripts' `zfs list` captures carry the named alarm** (`|| die`) instead of dying under `set -e` with raw zfs stderr, and the 4-VM e2e guards its guest `du` capture the way the 9-node one does.
- **`docs/design.md` G7 records that `modelfs pull` shipped in 0.7.0.**
...
process image without unmounting, so a binary upgrade no longer costs the
kernel connection, the peer port, or an fd an engine already holds; the
daemon moved to libfuse's low-level API to make that possible. `modelfs
pull` downloads a Hugging Face model revision straight onto the origin,
resumable, with the token never on argv. Documentation was restructured so
each topic has one home, and the six review guides copied from another Zig
project were retargeted at this tree.

On-disk formats and the peer wire are unchanged, so a mixed fleet with
`0.6.0` peers still fills. Two behavior changes are listed under
**Upgrade from 0.6.0** below.

### Documentation restructure - 2026-09-02
- **Every topic now has one home and the other files link to it.** The contributor script list stood in README, CONTRIBUTING, and `check.sh --help`; setup in README and CONTRIBUTING; the module table and the flag/environment reference in README and architecture.md; the docs index in README and docs/README.md. The scripts and setup now live in [CONTRIBUTING.md](CONTRIBUTING.md) (including the `run_benchmarks_and_plots.py --update-docs` note that earlier entries below place in README), the module table and configuration rules in [docs/architecture.md](docs/architecture.md), and the index in [docs/README.md](docs/README.md). README trades its ~350-word flag paragraph for a cheat-sheet table and points at `modelfs help`.
- **The two reference documents were reformatted, not rewritten.** architecture.md's grab-bag "Run" section split into Modules, Metadata and namespace operations, Command details, Configuration rules, and Logging; THREAT_MODEL.md's entry-point and mitigation tables became prose where cells had passed 1000 characters. Longest line: 2028 to 537 in the first, 2318 to 660 in the second. Every claim, citation, and caveat is preserved.
- **Six review guides were retargeted from the Zig project they were copied from.** `zig-idiomatic`, `zig-best-practices`, `zig-0.16-changelog`, `abstractions`, `simd`, and `net-send` each opened with a gate that skipped this tree on sight (`src/ecs/` must exist), so they could never run. They now gate on the mount tree and check it by name: `sys.zig` as the sanctioned syscall layer rather than a blanket ban on syscalls, `./scripts/check.sh` rather than `make check`, the flat `src/` layout and its downward import chain, the no-heap-on-hot-path rule, and the real symbols behind each verdict. `net-send-review.md` became the peer transfer contract (source selection, the `/stage`-to-`/data`-to-next-path-to-origin ladder, deadlines, saturation, counter coverage), and `simd-review.md` now says out loud that this tree is I/O bound and that a well-argued rejection list is the expected result. `docs/README.md` gains the guide-to-verdict table the four surviving guides needed anyway.

### Hugging Face pulls via `modelfs pull` - 2026-09-02
- **`modelfs pull <owner/repo> --origin PATH` downloads a model revision onto the NFS origin**, where every node's mount then serves it. Files already present at the size the listing reports are skipped, so a rerun after a failure resumes; each file lands as `.part` and is renamed only once its bytes are complete. `--revision` takes a branch, tag, or commit (default `main`); `--dest` sets where under the origin the files go (default: the repo id). The access token comes from `HF_TOKEN`, else `$HF_HOME/token`, else `~/.cache/huggingface/token` -- never a flag, because argv is world-readable through `/proc`, and core dumps are disabled for the run when one is in play. Repo ids, refs, and every path in the listing are validated before they reach a URL or a join against the origin (`src/hf.zig`).

### Binary hot-reload via `modelfs update` - 2026-09-02
- **`modelfs update` replaces the live mount's process image without unmounting.** The CLI looks up the daemon the same way `status` does (`status.json` pid plus a 120 s heartbeat), writes a request under the cache dir, and signals the live pid. The daemon inherits the FUSE connection and peer listen fds (no close-then-rebind, still no `SO_REUSEPORT`), reconstitutes origin/cache/id/piece/listen/watermarks/io and the cluster PSK from a sealed memfd (the secret never appears on argv), and attaches to the existing session. An fd opened before the swap keeps reading across it. A missing, dead, or wedged `status.json` is a named exit 1. `--cache` / `MODELFS_CACHE` locate the daemon; mount-only flags are refused. `scripts/test_hot_reload.sh` is the harness (needs `/dev/fuse`, so it runs locally).
- **The daemon now speaks libfuse's low-level API.** The high-level API keeps the inode table privately and aborts the process on an inode it does not know, which is exactly what a replacement image inherits from the kernel, so only a table modelfs owns can survive a handover. The path handlers are unchanged and are reached through an ino/fh layer that is snapshotted into the handover state. Two visible consequences: the connection's `FUSE_INIT` request is captured verbatim and replayed by the replacement (nothing derived from it round-trips -- a reconstructed one loses `FUSE_MAX_PAGES` and the connection then fails every read with EINVAL), and `-o max_idle_threads=8` is gone with the high-level session it belonged to.
- **Unlinking an open file now removes it, instead of renaming it to `.fuse_hiddenNNNN` on the origin.** The old behavior came from libfuse's high-level default. On a shared NFS origin those hidden files are visible to every other node and orphaned if the node that made one dies. Reads through an fd whose name was unlinked now fail with ENOENT.

### Upgrade from 0.6.0 - 2026-09-02
- **Unlinking a file another process still has open now removes it.** libfuse's high-level API renamed it to `.fuse_hiddenNNNN` on the origin first; the low-level API this release moves to does not, and on a shared NFS origin those hidden files were visible to every other node and orphaned if the node that made one died. Reads through an fd whose name was unlinked now fail with ENOENT rather than continuing to serve.
- **The mount no longer passes `-o max_idle_threads=8`.** That option belonged to the high-level session this release replaces; libfuse's own default applies instead. No flag changed, and nothing reads the value.

...
You are a senior networking engineer whose task is to review the peer send and fetch path in `src/peer.zig` and `src/discover.zig`.
...
- Applicability gate: confirm this is the modelfs **mount** tree: `build.zig.zon`, `src/peer.zig`, `src/discover.zig`, `src/store.zig`, and `src/sys.zig` must exist; `src/ecs/` must not exist. On any miss, print the skip result and stop.
...
2. **The fallback ladder runs in one direction and never loops.** `/data` on a peer, then the next path, then the origin. Re-entering a rung already tried, or falling all the way back without counting the failure, is P1.
...
8. **Partial transfers are looped, not assumed.** `sys.sendfileAll` and the `preadAll`/`pwriteAll`/`writeAll` family return `-errno` and must be driven to completion or to a real error. Treating a short return as success is P0.
9. **Preserve the sendfile fast path.** Trace `serveData` through `verifyRange` and `streamRange` in `src/peer.zig`. Normal response transmission uses `sys.sendfileAll`; replacing that branch with read-into-buffer-then-write is P1. Zero-copy does not prohibit `verifyRange` reading cached bytes into user space to hash them, or `streamRange`'s bounded-buffer fallback before any body bytes have been sent. Preserve both; neither is a finding merely for copying bytes. Hash verification belongs to `zig-src-review.md` item 7. Preserve the `Cached.xfer` guard across verification and transmission: dropping it permits a cull to punch a hole mid-transfer and is P0.

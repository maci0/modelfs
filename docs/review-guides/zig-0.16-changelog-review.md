...
**E. `std.mem` naming: "index of" became "find".** `find`, `findScalar`, `findScalarLast`, `findPosLinear`, and the `cut` functions are the 0.16 names; `indexOf`, `indexOfScalar`, and `lastIndexOfScalar` survive as aliases the stdlib marks "Deprecated in favor of". Mixed use therefore compiles and cannot diverge in behavior, which makes it a readability finding, not a defect. **Measured 2026-09-03: 142 `std.mem.find*` call sites and no deprecated ones**, after the tree-wide rename. Re-count before reporting; if the old names have crept back, a wholesale rename is over the 200-line budget and belongs in its own scoped change rather than inside another finding.
...
Report in chat: scope (files covered, the pinned `minimum_zig_version`, date), a findings table, counts by severity, and an ordered fix plan, and a short note with the top findings and whether `./scripts/check.sh` was run after any fix.

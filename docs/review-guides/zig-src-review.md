...
2. Path containment: remote-supplied paths must pass `decodePath` (`src/peer.zig`) and then `relOk` (`src/store.zig`) before touching origin or cache. `/have` and `/data` share that pair, then `discover.relIsCluster` (404, no hydrate). Any consumer of the request target or query that slices a path directly, joins one into a root without `relOk`, or hydrates `.cluster` is P0.
...
11. Hot-path allocation: piece hydration and peer request parsing use stack buffers or one reusable piece-sized buffer; allocating functions take an explicit `gpa`. The per-request piece-sized buffer for hydration (`hydrateRange`) is that reusable buffer. A new `allocator.alloc` / `dupe` / `allocPrint` on the FUSE read/hydrate path (`src/fuse_fs.zig`) or `handleConn` serve path (`src/peer.zig`) that is not that buffer (a second heap copy of the same piece, or an alloc on request-parse) is a finding.
12. Test visibility: `src/root.zig` is the test aggregator (`test { _ = @import(...) }`). A `src/*.zig` other than `root.zig` and `c.zig` that is not imported from that block is invisible to `zig build test` and is a finding. `src/c.zig` is a maintained re-export, not generated output; `build.zig` supplies its imported `c` module via `addTranslateC`. Fix binding declarations in the maintained headers or `build.zig`, never in the generated module. A missing `src/root.zig` is itself the finding.

If available, use: `rg -n` to locate each pattern before judging (`bearerOk`, `relOk`, `decodePath`, `relIsCluster`, `resolveRel`, `clientCreateMode`, `parseLease`, `expectedHash`, `psk-value|psk_value`, `@cImport`, `catch unreachable|catch \{\}`, `std\.os\.linux\.`, `allocator\.(alloc|dupe)|allocPrint` under `src/`), then read the surrounding function and trace the call path from the FUSE op or `handleConn`; a search hit alone is not proof (tests and counted drop paths also match these patterns). Confirm `src/root.zig` imports every other `src/*.zig` except `c.zig`.

...
7. Gate options stay: the optional shellcheck checks live as `enable=` directives in `.shellcheckrc` (the `shellcheck scripts/*.sh` line in `scripts/check.sh` itself carries no flags), and do not exclude a new `scripts/*.sh` or `scripts/*.py` from the `scripts/` globs those linters already use, to make a script pass.

...
1. `lib.sh` is the root and scratch source: every `scripts/*.sh` except `lib.sh` itself sources `lib.sh` before using `ROOT_DIR` or `SCRATCH_DIR`. A new shell script that hardcodes a repo-relative path or a scratch location instead of those variables is a finding. A script that never uses those variables (today: `install_libfuse3_dev.sh`) is exempt from the source requirement.

...
| Cluster PSK | `/etc/modelfs.psk` or `--psk-value`, process memory, every request's `Authorization` header (src/main.zig:350-408, src/peer.zig:655-671) | Full impersonation of any node: read every weight, serve poisoned pieces |
...
| Cache integrity | `data/` sparse files + `meta/*.pieces` bitfields | Punched holes read as zeros; without verification, zeros and attacker bytes are indistinguishable from real weights (sidecars trusted at load src/store.zig:375-412; cull loop src/fuse_fs.zig:602-644) |
...
| FUSE operations on the mountpoint | ops table src/fuse_fs.zig:769-787; path policy src/fuse_fs.zig:91-95 | Paths, write buffers, modes from every local process that can reach the mount |
| Lease files `<origin>/.cluster/<id>.json` written by other nodes | refresh src/discover.zig:430-504, sweep src/discover.zig:513-538 | JSON documents: ids, expiry timestamps, address lists (parsed src/proto.zig:180-182) |
...
| CLI subcommands and flags | src/main.zig:410-509 dispatch, parseArgs src/main.zig:182-348 | Paths, addresses, sizes, watermarks, id, PSK source |
| Environment variables `MODELFS_ORIGIN/CACHE/PSK/PSK_VALUE/ID` | src/main.zig:198-207 | Same values as their flags; explicit flag wins |
| PSK file (`--psk`, default `/etc/modelfs.psk`) | loadPsk src/main.zig:350-408 | Up to 4096 bytes, whitespace-trimmed |
| Cache-dir artifacts read back at runtime (`meta/*.pieces` bitfields, `pin/` markers, `status.json`) | store walk/load src/store.zig, status read src/main.zig:790-820 | On-disk state written by this same process; trusted structurally, not cryptographically |
...
**B1: local processes to daemon (FUSE).** Authority transition: anything crossing the mount becomes daemon-uid I/O against origin and cache. Enforcement is delegated to the kernel via `default_permissions` mounted by default; `allow_other` is opt-in (src/main.zig:697-700). Path policy is centralized in `resolveRel` (src/fuse_fs.zig:91-95): `.cluster` is invisible (lookup gets ENOENT, mutation EPERM), traversal and absolute paths get EPERM via `relOk`.
...
**Secrets flow.** The PSK enters via file read at startup (mode-checked, group/world-readable warned src/main.zig:398-400) or `--psk-value` (argv; warned as world-readable via /proc src/main.zig:353-364). It lives for the process lifetime in memory and leaves the host on every outbound peer request in a plaintext header (src/peer.zig:655-671) and implicitly to any passive listener on inbound connections. Empty PSKs are refused before binding (src/main.zig:353-357, 402-406). Rotation means regenerating the file on every node simultaneously; there is no versioning, overlap window, or revocation.
...
| Threat | Assessment |
|---|---|
| S spoofing another local user | Delegated to kernel permission checks (`default_permissions`, src/main.zig:699); root bypasses everything by definition |
...
| I disclosure across users | Governed by origin/cache file modes (cache data created 0644, src/store.zig:966; origin create/mkdir modes arrive from the client but are masked to permission bits, src/fuse_fs.zig:104-106); `.cluster` hidden but its secrecy is irrelevant (no PSK inside leases) |
...
| E elevation of privilege | Path escape blocked at `resolveRel`/`relOk` (src/fuse_fs.zig:68-95, src/store.zig:1221); symlink redirection of staged writes blocked by O_NOFOLLOW (src/sys.zig:300-313); client-supplied create/mkdir modes stripped of setuid/setgid/sticky so a mount writer cannot plant a daemon-owned special-bit executable (clientCreateMode src/fuse_fs.zig:97-106, applied at 186 and 469); the disk-cull walk samples entries with lstat and skips symlinks so planted links can neither be punched through nor descended into to steer fallocate punches outside the cache tree (src/store.zig:1104-1145, src/sys.zig:236-238) |
...
| E elevation via path tricks | `..`, absolute, control-byte, NUL paths refused with 400 (relOk gate src/peer.zig:317-322; test coverage src/store.zig:1323); unknown routes 404 before parameter parsing (src/peer.zig:303-311); non-GET 405 (src/peer.zig:279-284) |
...
| T tampering with other nodes' leases | Sweeping is mtime-based and skips only the sweeper's own id (src/discover.zig:513-538, self-skip 523); a co-tenant who can touch mtimes can evict live nodes from discovery. Requires origin write access, same precondition as above |
...
| D discovery poisoning | Corrupt/expired leases are skipped, not fatal (src/discover.zig:452-465); a flood of garbage lease files costs readdir+parse per tick, unbounded by count (minor, origin-write precondition) |
...
| Empty-PSK refusal at startup | src/main.zig:353-357, 402-406 | Accidental unauthenticated service |
| PSK file permission warning (group/other bits) | src/main.zig:398-400 | Local PSK theft by co-users (detection, not prevention) |
...
| Path safety gate `relOk` applied at every external path boundary (FUSE, peer HTTP, CLI pin) | src/store.zig:1221; call sites src/peer.zig:317-322, src/fuse_fs.zig:68-95, src/main.zig:947-953 | B1/B2/E: traversal out of origin and cache roots; log/terminal injection via control bytes |
...
| Client-supplied FUSE create/mkdir modes masked to permission bits, stripping setuid/setgid/sticky before any origin create | src/fuse_fs.zig:97-106, applied at mf_create 186 and mf_mkdir 469 | B1/E: planting daemon-owned special-bit executables through the mount |
| Symlink-planting defense (O_NOFOLLOW) on staged and cache writes | src/sys.zig:300-313, src/discover.zig:405-423, src/store.zig:960-969, src/fuse_fs.zig:765 | Local attackers redirecting privileged writes |
| Disk-cull walk samples cache entries with lstat and skips symlinks entirely (neither sampled as punch victims nor descended into), with a depth cap on nesting | src/store.zig:1104-1145, src/sys.zig:236-238 | B1/E: planted symlinks in a writable cache tree steering fallocate punches at arbitrary daemon-writable files outside `data/` |
| Untrusted-name hygiene: printable gates before echoing lease names/ids to logs and terminal output | src/discover.zig:128-149, src/main.zig:913-920 | Log forgery and terminal escape injection from crafted lease names |
...
| CLI flag scoping: mount-only options refused on status/peers/pin and positional arity enforced at parse (exit 2), instead of accepted-and-ignored invocations | rejectOutsideMount src/main.zig:146-151; arity gates src/main.zig:473-503 | CLI entry point: silent no-op runs that leave an operator believing an option took effect |
| Lease validation: expired filtered, corrupt skipped, self skipped, id charset enforced at publish and hostname fallback, dotted-quad enforcement on advertised addresses | src/discover.zig:452-465, 158-166, 17-40; src/main.zig:330-336, 297-301 | Discovery self-partitioning and malformed-document handling on B3 |
| Stale status.json retirement via pid liveness | src/main.zig:777-784, 790-820 | Monitoring deception by crash leftovers |
| Failure-only per-event logs plus tick summary counters (http401/http5xx/httpbad/probe_err) | src/peer.zig:259-297, src/fuse_fs.zig:673-714 | Detecting auth failures, malformed-request storms, silent cluster degradation |
...
- **`relOk` is the only path-safety control** for every externally supplied path (three call sites above). It is well tested (src/store.zig:1323) and centrally defined, which is the right shape, but any new entry point that forgets it loses all path containment.
...
3. **Static shared PSK, no rotation or revocation (R3).** One credential authenticates every node forever (loadPsk src/main.zig:350-408; no expiry, no identity). Node compromise equals cluster compromise; departure of a node requires cluster-wide key regeneration by hand.
...
3. **Route hijack for interception.** Publish a lease advertising the attacker's IP with high `mbps` (prior conversion src/discover.zig:346-351); victims preferentially connect and present the PSK; the attacker now sees (and can alter) fetch traffic selectively.
...
5. **Cache-disk exhaustion on a victim.** Continuously fetch distinct large ranges so the victim's cache fs fills with attacker-chosen pieces; culling (src/fuse_fs.zig:602 cullLoop) fights back but pinned files and active transfers are exempt (cullOne skip conditions, src/store.zig:983-1027), so steady pressure raises IO load and evicts useful pieces (denial of cache).

Trust placed in client-side enforcement: none found. The server validates path, method, range, and auth independently; clients trust peer-supplied bitmaps only for routing, and stale bits degrade to origin fallback rather than wrong data (probe-cache design, src/discover.zig:218-285, X-Piece-Size gate src/peer.zig:1040-1044).

---

## Response readiness (notes only)

- **Audit trail:** per-node journald logs carry failure-only events, each 401 with its source address (src/peer.zig:292-293), failed fetches with ip:port, origin errors, plus the tick counters; status.json exposes lifetime aggregates (tick summary src/fuse_fs.zig:673-714, status write src/fuse_fs.zig:716-767). Still no persistent, centralized record, and successful requests are unattributed; o11y-review owns log structure.

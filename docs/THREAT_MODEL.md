...
| Covers | modelfs daemon (`mount`) and CLI as of `v0.7.0`, peer HTTP protocol, lease discovery, FUSE surface |
...
**Vulnerability handling.** [SECURITY.md](../SECURITY.md) names the supported version (`v0.7.0`
is current; the `0.7.x` line receives security fixes) and the route from report to shipped fix.

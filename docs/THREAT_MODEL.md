...
| R8 | Crash-time PSK spill: a daemon crash used to write process memory holding the secret wherever `kernel.core_pattern` points | secrets to code | Mitigated (`disableCoreDumps` zeros `RLIMIT_CORE` at mount; residual: `setrlimit` failure is ignored) |
...
| Mount zeros `RLIMIT_CORE` after loading the PSK, drops `MODELFS_PSK_VALUE` from the process environment so the `auto_unmount` helper cannot inherit it, and `secureZero`s the in-memory copy on teardown | `disableCoreDumps` / `scrubPskEnv` src/sys.zig, called from cmdMount src/main.zig | Crash-time core of the daemon no longer contains the secret (closes former gap R8). Residual: `setrlimit` failure is ignored, and the secret still lives in process memory for the mount's lifetime |

...
| Path safety gate `relOk` applied at every external path boundary (FUSE, peer HTTP, CLI pin) | src/store.zig; call sites src/peer.zig, src/fuse_fs.zig, src/main.zig | B1/B2/E: traversal out of origin and cache roots; log/terminal injection via control bytes (C0, DEL, UTF-8 C1, U+2028/U+2029 via `containsControl` src/proto.zig) |

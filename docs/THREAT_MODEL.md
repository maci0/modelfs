...
| E elevation of privilege | Path escape blocked at `resolveRel`/`relOk` (src/fuse_fs.zig, src/store.zig); symlink redirection of staged writes blocked by O_NOFOLLOW (src/sys.zig). Syscalls with no O_NOFOLLOW (chmod, statvfs, readdir) lstat first and return ELOOP on a final-component symlink: `mf_chmod`/`mf_readdir` src/fuse_fs.zig, `originStatvfs` src/store.zig (a planted link must not route the daemon's chmod onto a setuid target, list a client-local directory through the mount, or report the host root's size/free via `df`). `statOrigin` is lstat (src/store.zig); `mf_open` answers ELOOP on S_IFLNK. Client-supplied create/mkdir/chmod modes stripped of setuid/setgid/sticky (`clientCreateMode` src/fuse_fs.zig, applied at mf_create, mf_mkdir, and mf_chmod). No FUSE `symlink`/`mknod`/`link` handlers (`ops` src/fuse_fs.zig): a mount user cannot plant origin links through FUSE (origin write on the NFS export is B3). Disk-cull walk samples with lstat and skips non-regular entries, so planted links can neither be punched through nor descended into (`walkData` src/store.zig) |
...
Former abuse case (read `status.json` as another uid) is closed: the artifact is 0600 (`writeFileOwnerOnly` src/sys.zig). Cross-uid weight theft stays closed by 0600 files / 0700 dirs.

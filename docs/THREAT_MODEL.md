...
| I disclosure across users | Governed by origin/cache file modes (cache data created 0600, leftover 0644 tightened on the next open, src/store.zig; origin create/mkdir/chmod modes arrive from the client but are masked to permission bits, src/fuse_fs.zig); `.cluster` hidden but its secrecy is irrelevant (no PSK inside leases) |

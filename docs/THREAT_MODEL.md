...
| Operational state (`status.json`) | cache root, created 0644 (`writeFileFull` src/sys.zig) | A local uid that can read the cache root learns pid, peer count, cache fill, and `origin_down`; not weights or the PSK |
...
| I disclosure across users | Cache weights: data files created 0600, leftover 0644 tightened on the next open (`cache_data_mode` src/store.zig); `data/`/`meta/`/`pin/` and nested parents created 0700 (`cache_dir_mode` src/store.zig) so a uid blocked by origin modes cannot list which weights are cached or pinned. Origin create/mkdir/chmod modes arrive from the client but are masked to permission bits (`clientCreateMode` src/fuse_fs.zig). `.cluster` is hidden; its secrecy is irrelevant (no PSK inside leases). Residual: `status.json` is 0644 (`writeFileFull` src/sys.zig) at the cache root, which docs/operations.md `mkdir -p`s without a mode (world-searchable under a default umask), so any local uid can read pid, peer count, cache fill, and `origin_down` |
...
| Cache data 0600, cache dirs 0700 | `cache_data_mode` / `cache_dir_mode` src/store.zig (`openCache` fchmod leftover 0644 to 0600; `ensureLayout`/`setPin`/sidecar mkdirAll use 0700) | B1/I: a local uid blocked by origin modes and FUSE `default_permissions` cannot read cached weights or list which nested paths are cached or pinned |
...
8. **`status.json` is world-readable on a default cache root (B1/I residual).** The file is created 0644 (`writeFileFull` src/sys.zig) at the cache root, and docs/operations.md `mkdir -p`s `/var/cache/modelfs` without a mode (world-searchable under a default umask). Any local uid can read pid, peer count, cache fill, and `origin_down`. Weights stay behind 0600 files / 0700 dirs. Same-uid as the daemon can already ptrace the live secret (secrets flow); this row is the cross-uid leftover.
...
What a hostile actor can do, with the enabling path named. Cases 1–5 need the PSK (a legit-but-curious node, a compromised spark, or anyone who captured it off the wire per R1). Case 6 needs write access to the cache directory. Case 7 needs only read access to the cache root.
...
7. **Read cluster operational state, locally.** `status.json` is 0644 at the cache root (`writeFileFull` src/sys.zig; docs/operations.md `mkdir -p` of `/var/cache/modelfs` with no mode). Any local uid that can search the cache root can learn the daemon pid, peer count, cache fill, and `origin_down` without FUSE access or the PSK. Cross-uid weight theft is closed by 0600 files / 0700 dirs; this leftover is topology and liveness only.

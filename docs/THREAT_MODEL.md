...
**B2: network to peer server.** Every GET, including `/ping`, requires the bearer PSK, checked before route dispatch (src/peer.zig); comparison is timing-safe SHA-256 equality (src/proto.zig). Non-GET answers `405 Allow: GET` without consulting the token (`handleConn` src/peer.zig), so an unauthenticated probe still learns that a peer service is present and that GET is the only method. After auth, the request path is URL-decoded into a fixed buffer and gated by `relOk` before touching origin or cache roots (src/peer.zig). There is no per-node identity: possessing the PSK is membership. Binding is always `0.0.0.0` on each unique advertised port (`bindAll` src/peer.zig); `--listen 127.0.0.1:18080` does not confine the listener to loopback.
...
| I information disclosure | Weights and PSK readable on path (gap R1); error replies carry empty bodies and no internals (src/peer.zig); log lines include rel paths only after relOk strips control bytes. Unauthenticated non-GET still receives `405 Allow: GET` (handleConn src/peer.zig), confirming a live peer listener |
...
|---|---|---|
| Bearer PSK required on every GET including /ping (non-GET answers 405 before the token is consulted) | src/peer.zig handleConn | Raises B2 GET entry bar to PSK possession; does not hide that a listener is present |
...
| CLI flag scoping: mount-only options refused on status/peers/pin, positional arity enforced at parse (exit 2), percentages clamped to 0..100, watermark ordering validated cross-field, `--origin` required to be an existing directory for both consumers (a regular file realpaths fine but can never hold leases or serve joined reads), and any `MODELFS_*` name outside the documented set refused as a typo | rejectOutsideMount src/main.zig; positional gates src/main.zig; percent gate takePercent and the cull.ordered watermark-ordering gate in parseArgs src/main.zig; pathIsDir gates at cmdMount and cmdPeers src/main.zig; checkKnownEnv src/main.zig | CLI entry point: silent no-op runs that leave an operator believing an option took effect; harness knobs cannot squat on the daemon's env namespace |
...
| Status.json retirement on two axes: an exited pid reads as not running, and a live pid whose artifact stopped ticking past 120 s (eleven missed discovery ticks) reads as not serving, so a wedged daemon cannot report healthy to monitors keying on `status`'s exit code (`now_s` stamp published by statusJson src/fuse_fs.zig; pidAlive, max_status_age_secs, and the age gate in cmdStatus src/main.zig) | src/main.zig, src/fuse_fs.zig | Monitoring deception by crash leftovers and by hung-but-alive daemons |
| Failure-only per-event logs plus tick summary counters (http_unauthorized/http_5xx/http_malformed/http_dropped/probe_err) | src/peer.zig, src/fuse_fs.zig | Detecting auth failures, malformed-request storms, saturation at the handler cap, silent cluster degradation |
| Fuzz harnesses over every untrusted-input parser: request heads and peer replies (auth/path/range pipeline), lease JSON documents, the URL codec pair across the trust boundary, the FUSE path gate, and parseV4 diffed against libc inet_pton across the whole input space; seed corpora share one framing helper (src/fuzzcorpus.zig) | src/peer.zig; src/proto.zig; src/discover.zig; src/fuse_fs.zig | Regression resistance on the B1/B2/B3 parsers; drift between ingestion and dial gates |
...
- **Audit trail:** per-node journald logs carry failure-only events, each 401 with its source address (src/peer.zig), failed fetches with ip:port, origin errors, pin/unpin state changes (persistent culling-exempt state is answerable from the journal alone, cmdPin src/main.zig), plus the tick counters including `http_dropped` saturation; status.json exposes lifetime aggregates and a wall-clock liveness stamp (tick summary src/fuse_fs.zig, status write src/fuse_fs.zig). Still no persistent, centralized record, and successful peer requests are unattributed; o11y-review owns log structure.

...
| Cluster PSK | `/etc/modelfs.psk` or `--psk-value`, process memory, every request's `Authorization` header (src/main.zig:329-387, src/peer.zig:655-671) | Full impersonation of any node: read every weight, serve poisoned pieces |
...
| Cache integrity | `data/` sparse files + `meta/*.pieces` bitfields | Punched holes read as zeros; without verification, zeros and attacker bytes are indistinguishable from real weights (sidecars trusted at load src/store.zig:375-410; cull loop src/fuse_fs.zig:602-638) |
...
| FUSE operations on the mountpoint | ops table src/fuse_fs.zig:762-780; path policy src/fuse_fs.zig:91-95 | Paths, write buffers, modes from every local process that can reach the mount |
| Lease files `<origin>/.cluster/<id>.json` written by other nodes | refresh src/discover.zig:432-507, sweep src/discover.zig:516-541 | JSON documents: ids, expiry timestamps, address lists (parsed src/proto.zig:180-182) |
...
| CLI subcommands and flags | src/main.zig:389-487 dispatch, parseArgs src/main.zig:176-328 | Paths, addresses, sizes, watermarks, id, PSK source |
| Environment variables `MODELFS_ORIGIN/CACHE/PSK/ID` | src/main.zig:177-186 | Same values as their flags; explicit flag wins |
| PSK file (`--psk`, default `/etc/modelfs.psk`) | loadPsk src/main.zig:329-387 | Up to 4096 bytes, whitespace-trimmed |
| Cache-dir artifacts read back at runtime (`meta/*.pieces` bitfields, `pin/` markers, `status.json`) | store walk/load src/store.zig, status read src/main.zig:763-793 | On-disk state written by this same process; trusted structurally, not cryptographically |
...
**B1: local processes to daemon (FUSE).** Authority transition: anything crossing the mount becomes daemon-uid I/O against origin and cache. Enforcement is delegated to the kernel via `default_permissions` mounted by default; `allow_other` is opt-in (src/main.zig:672-673). Path policy is centralized in `resolveRel` (src/fuse_fs.zig:91-95): `.cluster` is invisible (lookup gets ENOENT, mutation EPERM), traversal and absolute paths get EPERM via `relOk`.
...
**Secrets flow.** The PSK enters via file read at startup (mode-checked, group/world-readable warned src/main.zig:377-380) or `--psk-value` (argv; warned as world-readable via /proc src/main.zig:337-342). It lives for the process lifetime in memory and leaves the host on every outbound peer request in a plaintext header (src/peer.zig:655-671) and implicitly to any passive listener on inbound connections. Empty PSKs are refused before binding (src/main.zig:333-336, 382-385). Rotation means regenerating the file on every node simultaneously; there is no versioning, overlap window, or revocation.
...
| Threat | Assessment |
|---|---|
| S spoofing another local user | Delegated to kernel permission checks (`default_permissions`, src/main.zig:672); root bypasses everything by definition |
...
| T tampering with other nodes' leases | Sweeping is mtime-based and skips only the sweeper's own id (src/discover.zig:516-541, self-skip 526); a co-tenant who can touch mtimes can evict live nodes from discovery. Requires origin write access, same precondition as above |
...
| D discovery poisoning | Corrupt/expired leases are skipped, not fatal (src/discover.zig:461-468); a flood of garbage lease files costs readdir+parse per tick, unbounded by count (minor, origin-write precondition) |
...
| Empty-PSK refusal at startup | src/main.zig:333-336, 382-385 | Accidental unauthenticated service |
| PSK file permission warning (group/other bits) | src/main.zig:377-380 | Local PSK theft by co-users (detection, not prevention) |
...
| Path safety gate `relOk` applied at every external path boundary (FUSE, peer HTTP, CLI pin) | src/store.zig:1222; call sites src/peer.zig:317-322, src/fuse_fs.zig:68-95, src/main.zig:921-926 | B1/B2/E: traversal out of origin and cache roots; log/terminal injection via control bytes |
...
| Symlink-planting defense (O_NOFOLLOW) on staged and cache writes | src/sys.zig:300-313, src/discover.zig:407-425, src/store.zig:960-969, src/fuse_fs.zig:758 | Local attackers redirecting privileged writes |
| Disk-cull walk samples cache entries with lstat and skips symlinks entirely (neither sampled as punch victims nor descended into), with a depth cap on nesting | src/store.zig:1105-1146, src/sys.zig:236-238 | B1/E: planted symlinks in a writable cache tree steering fallocate punches at arbitrary daemon-writable files outside `data/` |
| Untrusted-name hygiene: printable gates before echoing lease names/ids to logs and terminal output | src/discover.zig:128-149, src/main.zig:889-893 | Log forgery and terminal escape injection from crafted lease names |
...
| Lease validation: expired filtered, corrupt skipped, self skipped, id charset enforced at publish and hostname fallback, dotted-quad enforcement on advertised addresses | src/discover.zig:461-468, 158-166, 17-40; src/main.zig:309-315, 271-282 | Discovery self-partitioning and malformed-document handling on B3 |
| Stale status.json retirement via pid liveness | src/main.zig:753-757, 763-793 | Monitoring deception by crash leftovers |
| Failure-only per-event logs plus tick summary counters (http401/http5xx/httpbad/probe_err) | src/peer.zig:259-297, src/fuse_fs.zig:666-707 | Detecting auth failures, malformed-request storms, silent cluster degradation |
...
3. **Static shared PSK, no rotation or revocation (R3).** One credential authenticates every node forever (loadPsk src/main.zig:329-387; no expiry, no identity). Node compromise equals cluster compromise; departure of a node requires cluster-wide key regeneration by hand.
...
3. **Route hijack for interception.** Publish a lease advertising the attacker's IP with high `mbps` (prior conversion src/discover.zig:348-353); victims preferentially connect and present the PSK; the attacker now sees (and can alter) fetch traffic selectively.

...
| Cluster PSK | `/etc/modelfs.psk` or `--psk-value`/`MODELFS_PSK_VALUE`, process memory, every request's `Authorization` header (src/main.zig:389-447, src/peer.zig:697-713) | Full impersonation of any node: read every weight, serve poisoned pieces |
...
| Cache integrity | `data/` sparse files + `meta/*.pieces` bitfields | Punched holes read as zeros; without verification, zeros and attacker bytes are indistinguishable from real weights (sidecars trusted at load src/store.zig:375-413; cull loop src/fuse_fs.zig:907-950) |
...
| FUSE operations on the mountpoint | ops table src/fuse_fs.zig:1077-1095; path policy src/fuse_fs.zig:86-95 | Paths, write buffers, modes from every local process that can reach the mount |
| Lease files `<origin>/.cluster/<id>.json` written by other nodes | publish+refresh src/discover.zig:392-522, sweep src/discover.zig:531-565; also parsed for the `modelfs peers` listing src/main.zig:909-946 | JSON documents: ids, expiry timestamps, address lists (parsed src/proto.zig:162-164) |
| Origin file tree (model data served to peers and locally) | statOrigin src/store.zig:248-252; hydration src/peer.zig:412-484 | File bytes and sizes as they are on the NFS export |
...
| Cache-dir artifacts read back at runtime (`meta/*.pieces` bitfields, `pin/` markers, `status.json`) | store walk/load src/store.zig, status read src/main.zig:835-865 | On-disk state written by this same process; trusted structurally, not cryptographically |
...
**B2: network to peer server.** Every endpoint requires the bearer PSK, checked before route dispatch so `/ping` is covered too (src/peer.zig:304-316); comparison is timing-safe SHA-256 equality (src/proto.zig:127-136). After auth, the request path is URL-decoded into a fixed buffer and gated by `relOk` before touching origin or cache roots (src/peer.zig:331-341). There is no per-node identity: possessing the PSK is membership.

**B3: daemon to origin (shared directory).** The origin is semi-trusted storage that also carries control data from other nodes. Untrusted inputs here: lease JSON (ids, addrs), file names under `.cluster`, and model bytes themselves. Controls: expired/corrupt/self leases skipped (src/discover.zig:476-486), lease ip fields validated by a hand parser before socket use (parseV4 src/discover.zig:22-49; inet_pton again at dial src/peer.zig:738-745), O_NOFOLLOW on every staged write into shared or locally-writable dirs (lease staging src/discover.zig:423-435, cache data src/store.zig:948-973, pin markers and status.json src/sys.zig:302-315), control-byte filtering before any untrusted name/id is echoed to logs or terminals (printable/displayName src/discover.zig:143-167, applied in `peers` output src/main.zig:961-964 and refresh/sweep logging src/discover.zig:470-478, 556-563).
...
**Secrets flow.** The PSK enters via file read at startup (mode-checked, group/world-readable warned src/main.zig:437-440) or `--psk-value`/`MODELFS_PSK_VALUE` (argv/env; the argv spelling warned as world-readable via /proc src/main.zig:397-403). It lives for the process lifetime in memory and leaves the host on every outbound peer request in a plaintext header (src/peer.zig:697-713) and implicitly to any passive listener on inbound connections. Empty PSKs are refused before binding (src/main.zig:392-396, 441-445). Rotation means regenerating the file on every node simultaneously; there is no versioning, overlap window, or revocation.
...
| I disclosure across users | Governed by origin/cache file modes (cache data created 0644, src/store.zig:959; origin create/mkdir modes arrive from the client but are masked to permission bits, src/fuse_fs.zig:97-106); `.cluster` hidden but its secrecy is irrelevant (no PSK inside leases) |
...
| E elevation of privilege | Path escape blocked at `resolveRel`/`relOk` (src/fuse_fs.zig:66-95, src/store.zig:1273-1290); symlink redirection of staged writes blocked by O_NOFOLLOW (src/sys.zig:302-315); client-supplied create/mkdir modes stripped of setuid/setgid/sticky so a mount writer cannot plant a daemon-owned special-bit executable (clientCreateMode src/fuse_fs.zig:97-106, applied at mf_create 411 and mf_mkdir 714); the disk-cull walk samples entries with lstat and skips symlinks so planted links can neither be punched through nor descended into to steer fallocate punches outside the cache tree (src/store.zig:1097-1146, src/sys.zig:233-241) |
...
| T tampering with served/fetched bytes | No checksums anywhere on the wire or at rest; fetched pieces are cached and re-served verbatim (fill path src/peer.zig:1096-1144) (gap R2). Routing-level confusion (mixed piece grids) IS handled: mismatched `X-Piece-Size` answers are discarded (src/peer.zig:1070-1079) |
...
| I information disclosure | Weights and PSK readable on path (gap R1); error replies carry empty bodies and no internals (src/peer.zig:627-652); log lines include rel paths only after relOk strips control bytes |
| D denial of service | Handler cap 16 with claim-then-check accounting (src/peer.zig:15, 136-151); head read deadline 10 s defeats dribble-holds (src/peer.zig:175, 202-238); body deadlines scale with Content-Length (src/peer.zig:197-205, 817-818); oversized/malformed heads counted, not logged per-event, bounding log flooding (src/peer.zig:278-296). Remaining exposure: 16 slots is small enough to occupy with reconnect loops, and authenticated requests force per-piece origin reads + NVMe writes (gap R4/R6). Server-side allocation is range-bounded: ranges clamp to file size (src/peer.zig:589-598) and hydration uses one reusable piece-sized buffer (src/peer.zig:412-430) |
| E elevation via path tricks | `..`, absolute, control-byte, NUL paths refused with 400 (relOk gate src/peer.zig:331-341; test coverage src/store.zig:1426-1474); unknown routes 404 before parameter parsing (src/peer.zig:326-330); non-GET 405 (src/peer.zig:298-303) |

### B3: daemon to origin (.cluster control plane)

| Threat | Assessment |
|---|---|
| S forging membership | Anyone with origin write access can publish a lease naming arbitrary ips/ports; victims will dial them. The forged peer still needs the PSK to answer /data, but the connection attempt itself hands the victim's PSK to the forged address in a Bearer header (sendRequest src/peer.zig:697-713). Lease mbps even lets the forged address win routing outright (prior from lease field, src/discover.zig:349-377) (gap R5) |
| T tampering with other nodes' leases | Sweeping is mtime-based and skips only the sweeper's own id (src/discover.zig:524-565, self-skip 541); a co-tenant who can touch mtimes can evict live nodes from discovery. Requires origin write access, same precondition as above |
...
| D discovery poisoning | Corrupt/expired leases are skipped, not fatal (src/discover.zig:476-486); a flood of garbage lease files costs readdir+parse per tick, unbounded by count (minor, origin-write precondition) |
...
|---|---|---|
| Bearer PSK required on every endpoint including /ping | src/peer.zig:304-316 | Raises B2 entry bar to PSK possession |
...
| Duplicate-bind refusal: listeners use SO_REUSEADDR only, never SO_REUSEPORT, so a second modelfs start against a live port fails loudly instead of silently splitting connections between two daemons (usually different PSKs) | src/peer.zig:46-52; regression test src/peer.zig:1435-1462 | B2/S: accidental or malicious co-tenant daemon sharing the port and harvesting requests meant for the real one |
| Source-attributed rejection logging: the accepted peer address is captured per connection and named in every 401 line; malformed-head scanner noise is counted (`http_malformed`) instead of logged per event so probes cannot flood the journal | src/peer.zig:112-116, 250-262, 305-315 (attribution); 278-296 (count-only scanner path) | Investigating wrong-PSK probing campaigns after the fact (closes former gap R8) |
| Path safety gate `relOk` applied at every external path boundary (FUSE, peer HTTP, CLI pin) | src/store.zig:1273-1290; call sites src/peer.zig:331-341, src/fuse_fs.zig:66-95, src/main.zig:993-998 | B1/B2/E: traversal out of origin and cache roots; log/terminal injection via control bytes |
...
| Client-supplied FUSE create/mkdir modes masked to permission bits, stripping setuid/setgid/sticky before any origin create | src/fuse_fs.zig:97-106, applied at mf_create 411 and mf_mkdir 714 | B1/E: planting daemon-owned special-bit executables through the mount |
| Symlink-planting defense (O_NOFOLLOW) on staged and cache writes | src/sys.zig:302-315, src/discover.zig:423-435, src/store.zig:948-973, src/fuse_fs.zig:1073 | Local attackers redirecting privileged writes |
| Disk-cull walk samples cache entries with lstat and skips symlinks entirely (neither sampled as punch victims nor descended into), with a depth cap on nesting | src/store.zig:1097-1146, src/sys.zig:233-241 | B1/E: planted symlinks in a writable cache tree steering fallocate punches at arbitrary daemon-writable files outside `data/` |
| Untrusted-name hygiene: printable gates before echoing lease names/ids to logs and terminal output | src/discover.zig:143-167, src/main.zig:961-964 | Log forgery and terminal escape injection from crafted lease names |
| Bounded parsing everywhere: overflow-safe size/range parsers, suffix ranges rejected, URL decode bounds-checked, fixed-size head buffers (server request head sized so any head sendRequest can emit fits: triple-encoded PATH_MAX rel plus a full-size bearer token plus framing, 16.5 KiB, src/peer.zig:177-185; client response heads 8 KiB), 10 s head deadline, Content-Length allocation cap (client side, 512 MiB), body deadlines scaled to length | src/proto.zig:17-59, 83-125; src/peer.zig:159-248 (deadline constants and head read), 665, 769 (client head buffers), 782-851 (shared body reader incl. length cap at 814) | Memory-exhaustion and slow-loris style holds on B2 |
| Concurrency cap: 16 handlers, atomic claim-then-check; probe concurrency capped to the same number | src/peer.zig:15, 136-151, 952-961 | Unbounded thread/connection growth |
...
| Range clamping to file size; one reusable piece-sized hydration buffer | src/peer.zig:589-598, 412-430 | Server-side allocation driven by attacker-chosen ranges |
| Regular-file gate before any cache work on both data endpoints: directories and other non-regular origin objects answer 404 on /have and /data alike, instead of reaching hydration's pread on a directory fd (which surfaced as a misattributed 502) | src/peer.zig:371-374, 585-588 | B2 validation: non-regular paths driving cache writes and wrong-status replies |
...
| Lease validation: expired filtered, corrupt skipped, self skipped, id charset enforced at publish and hostname fallback, dotted-quad enforcement on advertised addresses | src/discover.zig:461-487, 169-184, 22-49; src/main.zig:369-375, 330-341 | Discovery self-partitioning and malformed-document handling on B3 |
| Stale status.json retirement via pid liveness | src/main.zig:825-861 | Monitoring deception by crash leftovers |
| Failure-only per-event logs plus tick summary counters (http_unauthorized/http_5xx/http_malformed/probe_err) | src/peer.zig:278-315, src/fuse_fs.zig:978-1022 | Detecting auth failures, malformed-request storms, silent cluster degradation |
| Fuzz harnesses over every untrusted-input parser: request heads and peer replies (auth/path/range pipeline), lease JSON documents, the URL codec pair across the trust boundary, the FUSE path gate, and parseV4 diffed against libc inet_pton across the whole input space | src/peer.zig:2552-2675; src/proto.zig:355-399, 460-507; src/discover.zig:906-939, 1043-1129; src/fuse_fs.zig:173-220 | Regression resistance on the B1/B2/B3 parsers; drift between ingestion and dial gates |
...
- **`relOk` is the only path-safety control** for every externally supplied path (three call sites above). It is well tested (src/store.zig:1426-1474) and centrally defined, which is the right shape, but any new entry point that forgets it loses all path containment.
...
1. **Plaintext transport with inline credentials (R1).** Every request carries `Authorization: Bearer <psk>` in cleartext (src/peer.zig:697-713) and every piece moves unencrypted (streamRange src/peer.zig:498-570). Any host on path (same L2, any router between racks, anyone doing ARP spoofing) reads weights and captures the PSK. design.md §9 promised mTLS-or-token as v1 auth; shipped is token-only over plain TCP.
2. **No piece integrity verification (R2).** Fetched bytes go straight into the cache and are marked filled (fetchRangeInto src/peer.zig:724-728, called from fillFromPeers src/peer.zig:1096-1144; completeFill src/store.zig:728-732); they are then served to local engines and to other peers. A malicious or compromised peer (or on-path rewriter, per R1) injects arbitrary bytes that propagate fleet-wide and persist until culled. design.md §9's "blake3 on every chunk... never serve unverified bytes" did NOT ship (architecture.md lists blake3 under "What did not ship"). At-rest corruption is equally silent: hole zeros read as valid filled data whenever a bitfield says filled.
...
1. **Bulk weight exfiltration.** Enumerate paths (any relOk-clean string; 404 vs 502 distinguishes absent from broken, replyOriginStat src/peer.zig:645-652), then `GET /data?path=<rel>` with successive `Range` headers to pull entire files from any node, at line rate via sendfile (streamRange src/peer.zig:498-570). No quota, rate limit, or audit trail distinguishes this from normal fills.
2. **Silent model poisoning.** Answer a victim's `/data` fetch with crafted bytes; the victim caches them marked-filled (fetchRangeInto src/peer.zig:724-728, landed by fillFromPeers via completeFill src/store.zig:728-732) and re-serves them to peers and local engines. Repeat per piece to corrupt a model everywhere it is cached, with no detection point anywhere in the system (R2).
...
5. **Cache-disk exhaustion on a victim.** Continuously fetch distinct large ranges so the victim's cache fs fills with attacker-chosen pieces; culling (src/fuse_fs.zig:907 cullLoop) fights back but pinned files and active transfers are exempt (cullOne skip conditions, src/store.zig:976-1021; punchPiece's xfer guard, src/store.zig:911), so steady pressure raises IO load and evicts useful pieces (denial of cache).

Trust placed in client-side enforcement: none found. The server validates path, method, range, and auth independently; clients trust peer-supplied bitmaps only for routing, and stale bits degrade to origin fallback rather than wrong data (probe-cache design, src/discover.zig:202-303, X-Piece-Size gate src/peer.zig:1070-1079).

---

## Response readiness (notes only)

- **Audit trail:** per-node journald logs carry failure-only events, each 401 with its source address (src/peer.zig:305-315), failed fetches with ip:port, origin errors, plus the tick counters; status.json exposes lifetime aggregates (tick summary src/fuse_fs.zig:978-1022, status write src/fuse_fs.zig:1024-1075). Still no persistent, centralized record, and successful requests are unattributed; o11y-review owns log structure.

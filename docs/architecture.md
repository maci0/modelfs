...
Env: `MODELFS_ORIGIN` `MODELFS_CACHE` `MODELFS_PSK` `MODELFS_ID` set the same values as their flags, and `MODELFS_PSK_VALUE` carries an inline secret that no flag accepts; an explicit flag wins and an empty environment value counts as unset. The whole `MODELFS_` prefix belongs to this CLI: any other member is refused as a typo'd knob instead of being silently ignored, so test and drill harnesses spell their knobs `MODFS_*` (see `scripts/lib.sh`).

...
| `tank/models/.cluster/manifests/` | piece-hash manifests | derived from origin bytes; covered by the dataset snapshots | regenerated by origin fills and writes (`Store.publishManifest` in `src/store.zig`); not an independent backup of model bytes |
...
Fence clients before changing origin bytes. Stop readers, writers, downloads (including
`modelfs pull`, which bypasses FUSE), and their automatic restarts. Stop every `modelfs`
daemon, unmount the clients' FUSE and NFS mounts, and stop the corresponding automount units
so a path lookup cannot reconnect them. On the desktop, stop `cachefilesd` after unmounting
NFS. Do not use lazy unmount as proof that open handles are gone. Keep clients fenced until
copy-back, verification, and section C step 3's cache wipe are complete.

Select a **known-good snapshot from before the first bad write or deletion**, not simply the
newest snapshot: later snapshots preserve the damage too. List creation times, then set
`SNAP` to the exact name inspected. If the affected path is now a child dataset, list and
clone that dataset's snapshot; a parent clone does not contain child datasets.

```bash
zfs list -r -t snapshot -o name,creation -s creation tank/models
...
read-only and unshared, regardless of inherited pool properties. A hold does not prevent
cloning: leave snapshot holds in place.

```bash
zfs clone -o mountpoint=/export/modelfs-recover -o readonly=on \
  -o sharenfs=off -o sharesmb=off "${SNAP}" tank/recover
sha256sum /export/modelfs-recover/gguf/broken-model.gguf
```

Compare that checksum with a trusted pre-incident digest or upstream manifest before copying.
For a cloned child dataset, adjust the path relative to its root. Preserve the current state
with a named recursive snapshot before overwriting it; record that snapshot separately from
the known-good restore point.

```bash
zfs snapshot -r "tank/models@pre-restore-$(date -u +%Y%m%dT%H%M%SZ)" &&
  cp -a /export/modelfs-recover/gguf/broken-model.gguf /export/models/gguf/ &&
  sync -f /export/models/gguf/broken-model.gguf &&
  cmp /export/modelfs-recover/gguf/broken-model.gguf /export/models/gguf/broken-model.gguf
```

Stop on any failure and retain the clone for inspection. After successful verification,
unmount and destroy only `tank/recover`, then perform section C step 3 before reopening clients.
The snapshot interval is the spacing between recovery points, not the amount of good work
lost: corruption recovery can discard all changes since the selected known-good snapshot.
If no retained snapshot predates the incident, local snapshots cannot recover those bytes.

### C. Pool or NAS dead

Before receiving anything, fence clients as in section B and fence the old NAS from the
client network, even if it currently appears dead. Do not reuse its address or export until
it cannot return and accept writes. The restore script sets `sharenfs` before it returns;
it does not fence clients or the old server.

Order matters. Node caches may hold pieces **newer** than the restored snapshot.
`Store.getIdentified` in `src/store.zig` treats an older mtime on the same inode as NFS
attribute lag, not a rewrite. A rollback can therefore retain post-snapshot pieces even
after a daemon restart; wipe persisted caches while every daemon is stopped.
...
# 3. With all daemons stopped and client mounts/automounts still fenced,
#    save the pin list (section A), then wipe every derived cache.
#    Use the configured cache paths if they differ from these defaults.
rm -rf /var/cache/modelfs/*        # on every spark
rm -rf /var/cache/fscache/*        # on the desktop, cachefilesd stopped

# 4. Restart cachefilesd on the desktop, restore client mount/automount
#    units (fstab from operations.md), then restart modelfs and reapply pins.
...
| deletion, corruption | retained known-good autosnap | changes since the selected pre-incident snapshot; hourly recovery-point spacing | minutes for copy-back, plus diagnosis and verification |
...
* Permission to clone the selected snapshot; keep its `modelfs-dr` hold in place. Cloning does not require `zfs release`
